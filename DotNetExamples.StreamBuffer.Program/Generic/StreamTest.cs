...
namespace DotNetExamples.StreamBuffer.Program.Generic
{
        ...
        where T : IEquatable<T>
    {
        ...
        /// </summary>
        public IList<IGenerator<T>> GeneratorList;
        ...
        {
            Buffer = buffer;
            GeneratorList = new List<IGenerator<T>>();
        }
        ...
        public void Register(IGenerator<T> generator) => GeneratorList.Add(generator);
        ...
        public void Run(int fillCount = 5, int delay = 5)
        {
            ...
            Console.WriteLine("[{0}] tf Get({1}): null == {2}", DateTime.Now.ToFileTime(), index, Buffer.Get(index));
        ...
        /// <returns></returns>
        private IEnumerable<Task> NextRandomTask(IList<Task> list, Random random)
        {
            while (0 < list.Count)
            {
        ...
        /// <returns></returns>
        protected Task BuildGeneratorTask(IStreamBuffer<T> buffer, IList<IGenerator<T>> generatorList, Random random, int minDelay, int maxDelay) => new Task(() =>
        {
            lock (Buffer.SyncRoot)
            {
                foreach (IGenerator<T> generator in generatorList)
                {
